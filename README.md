...
- Send department notifications through Resend when email settings are added.
...
When `RESEND_API_KEY` is configured, department notifications are sent through Resend using the AP-configured subject/body template. `EMAIL_FROM` must be a verified sender in Resend; use `onboarding@resend.dev` only for limited testing.
